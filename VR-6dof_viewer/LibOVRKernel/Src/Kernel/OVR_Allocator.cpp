...
bool InterceptCRTMalloc::ReplaceCRTMalloc(
  ...
  // There isn't a good solution for dealing with case #2, because in that case we would be
  // overwriting
  // the bytes of the actual malloc implementation, this making it uncallable in the future (without
  // the
  // risky trick of modifying the bytes as we call them). Copying the malloc instruction memory to a
  // new
  // location is not an option because malloc's implementation will have relative addess redirects
  // which
...
// library has initialized, and are destroyed right before the C standard library is destroyed
// (after after all other
// app globals are destroyed). That way we can execute code before other global variable
// constructors are called and
// execute code after other global variable destructors are called. There are other init_seg
// directives that can be
// used, such as those below. The linker actually just goes in alphabetic order, so you could
...
// AllocatorAutoCreate instance below.
// However, we can prevent the linker from doing this via various techniques, such as dll-exporting
// the instance.
...
// REG_DWORD of 0 or 1.
      ...
      if (DebugPageHeapEnabled) {
        ...
        // that
        // it's calling _CrtIsValidHeapPointer, which is invalid and recommended against by
        // Microsoft themselves.
        // We need to deal with this nevertheless. The problem is that the VC++ concurrency library
        // is
        // calling _CrtIsValidHeapPointer on the default heap instead of the current heap
        // (DebugPageHeap).
        // So we modify the _CrtIsValidHeapPointer implementation to always return true. The primary
        // risk
        // with this change is that there's some code somewhere that uses it for a non-diagnostic
        // purpose.
        // However this os Oculus-debug-internal and so has no effect on any formally published
        // software.
  ...
  if (p) {
    ...
    (_MSC_VER <              \
     1900) // VS2013 doesn't expose _free_base, so use this delayed approach with VS2013.
        ...
        // heap is active and
        // we would need to call a dbg function. The best way for us to do that would be to
        // implement our own function
        // which works the same as _free_dbg(). However, for now we just add this pointer to the
        // delayed free list and
  ...
  if (p) {
    ...
    (_MSC_VER <              \
     1900) // VS2013 doesn't expose _aligned_free_base, so use this delayed approach with VS2013.
        ...
        // debug heap is active and
        // we would need to call a dbg function. The best way for us to do that would be to
        // implement our own function
        // which works the same as _aligned_free_dbg(). However, for now we just add this pointer to
        // the delayed free list and
        // simply free it the normal way later when we shutdown.
...
void* Allocator::ReallocDebug(void* p, size_t newSize, const char* file, unsigned line) {
  ...
  // allocated by malloc before we
  // did the override, then p belongs to the original malloc heap. We can attempt to reallocate it
  // here with our own heap or
  ...
  } else if (MallocRedirect) // Else p came from the CRT heap. It was likely malloc'd before we
  // redirected malloc.
  {
#if OVR_DEBUG_CRT_PRESENT || \
    (_MSC_VER <              \
     1900) // VS2013 doesn't expose _realloc_base, so use this delayed approach with VS2013.
#if OVR_DEBUG_CRT_PRESENT
...
bool HeapIterationFilterRPN::Compile(const char* filter) {
       ...
       ++instructionCount) // While reading each line until the end of the text...
  {
    ...
    size_t i;
    bool isOperandLine = _strnicmp(filter, "and", 3) &&
        _strnicmp(filter, "or", 2); // To consider: Find a cleaner way to discern which of the two
    // kinds of lines this is (operand or operation).

    if ((*filter == '\r') || (*filter == '\n') || (*filter == '/')) {
      // Ignore lines that are empty or begin with /
    } else if (
        isOperandLine && (sscanf_s(
                              filter,
                              "%11s %7s %255[^;]s",
                              tempDataType,
  ...
  if (p) {
    // Creating a scope for the lock
    {
      ...
      if (FreedBlockArrayMaxSize) // If we have a delayed free list...
      {
        ...
        // in an inaccessible state for later freeing.
        // We do this because we don't want those pages to be available again in the near future, so
        // we can detect use-after-free misakes.
        Block* pBlockNew;
...
// bytes
// of unused memory between the user allocated space and the end of the page. There is no way around
// having this.
// For example, a user allocation of 3 bytes with 8 byte alignment will leave 5 unused bytes at the
// end of the page.
// We optionally fill those unused bytes with a pattern and upon Free verify that the pattern is
// undisturbed.
// This won't detect reads or writes in that area immediately as with reads or writes beyond that,
// but it will
...
void* DebugPageHeap::AllocCommittedPageMemory(size_t blockSize) {
  ...
  if (OverrunPageEnabled) {
    ...
    // reserved
    // because the OS will generate a one-time-only gaurd page exception. We probabl don't want
    // this, as it's
...
void DebugPageHeap::DisablePageMemory(void* pPageMemory, size_t blockSize) {
  ...
  // decommit or free the pages.
  // However, this results in more committed physical memory usage than we would need if we instead
  // decommitted the memory.
...
int AllocatorReportStateDbgCmd(const std::vector<std::string>&, std::string* output) {
  ...
  if (allocator) {
    ...
    strStream << "Underlying heap: " << (debugPageHeapEnabled
                                             ? "debug page heap."
