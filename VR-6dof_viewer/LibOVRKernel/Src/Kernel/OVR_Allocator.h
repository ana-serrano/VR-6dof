...
struct AllocMetadata {
  ...
  uint32_t ThreadId; // ThreadId at the time of the allocation. May potentially be stale if the
  // thread has exited.
...
class Allocator {
 ...
 public:
  ...
  // value of IterateHeapBegin.
  // For a given thread, IterataHeapBegin must be followed by IterateHeapEnd before any new call to
  // IterateHeapBegin.
  //
  // Example usage:
...
//     the two.
//     Time > 100s\n                Push the result of this on the stack.
//     or"                          Pop the two (AllocSize AND result and Time result), push the
//     result of the OR of the two.
//
// Example 4:
//    ((Size > 10) && (Size < 32)) || ((Time > 100) && (Time < 200))
//        ->
...
//     Time > 100s\n                Push the result of this on the stack.
//     Time < 200s\n                Push the result of this on the stack.
//     and\n                        Pop the two (Time results), push the result the the AND of the
//     two.
//     or"                          Pop the two (AllocSize AND result and Time AND result), push the
//     result of the OR of the two.
//
...
//      substring check.
//      Line                     ==,<,<=,>,>=    <integer>
//      Time                     ==,<,<=,>,>=    <integer>[s]    time in nanoseconds (or seconds if
//      followed by s). A negative time means relative to now.
//      Count                    ==,<,<=,>,>=    <integer>       A negative count means to return
//      the last nth allocation(s).
//      AllocSize (or just Size) ==,<,<=,>,>=    <integer>
//      BlockSize                ==,<,<=,>,>=    <integer>
//      Tag                      ==,has          <string>        case insensitive. has means
//      substring check.
//      ThreadId                 ==,<,<=,>,>=    <integer>       <,<=,>,>= are usually useless but
//      provided for consistency with other integer types.
//      ThreadName               ==,has          <string>        case insensitive. has means
//      substring check.
//
...
//       space and the page (due to alignment requirements), we optionally fill these with guard
//       bytes.
...
//       can
//       simulate MEM_RESERVE by having an extra page that's PROT_NONE instead of MEM_RESERVE. Since
//       Unix
//       platforms don't commit pages pages to physical memory until they are first accessed, this
//       extra
...
//   nullptr.
//   All returned allocations are aligned on a power-of-two boundary of at least
//   DebugPageHeap::DefaultAlignment.
//   AllocAligned supports any alignment power-of-two value from 1 to 256. Other values result in
//   undefined behavior.
...
//   functions, else the behavior is undefined.
//   You may not Free a pointer a second time, else the behavior is undefined.
//   Free otherwise always succeeds.
//   Allocations made with AllocAligned or ReallocAligned must be Freed via FreeAligned, as per the
