...
//==============================
// ovrLexer::ovrLexer
ovrLexer::ovrLexer(const char* source) : ovrLexer(source, OVR_strlen(source), NULL) {}
...
void ovrLexer::CopyResult(char const* buffer, char* token, size_t const maxTokenSize) {
  ...
  if (IsQuote(*buffer)) {
    ...
    if (IsQuote(lastChar)) {
      // The first char and last char are single-byte quotes, we can now just step past the first
      // and not copy the last.
      char const* start = buffer + 1;
      len = OVR_strlen(start); // We do not care about UTF length here since we know the quotes are
      // a single bytes
