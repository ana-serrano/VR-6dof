...
class List {
 public:
  ...
  const ValueType* GetFirst() const {
    return (const ValueType*)Root.pNext;
  }
  const ValueType* GetLast() const {
    return (const ValueType*)Root.pPrev;
  }
  ValueType* GetFirst() {
  ...
  bool IsEmpty() const {
    return Root.pNext == (const T*)(const B*)&Root;
  }
