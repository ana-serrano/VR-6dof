  ...
  for (int i = 0; i < (int)size; path++) {
    if (path[0] == '/' || path[0] == '\\') {
      if (i == 0 || dest[i - 1] != separator) {
        ...
        path[0] == '.' && path[1] == '.' && i > 2 && dest[i - 1] == separator &&
        dest[i - 2] != '.') {
...
inline String GetCleanPathString(const char* path, const char separator = '/') {
  char buffer[MAX_PATH_LENGTH];
  GetCleanPath(buffer, path, separator);
  return String(buffer);
}
inline String
GetRelativePathString(const char* path, const char* relativeTo, const char separator = '/') {
  char buffer[MAX_PATH_LENGTH];
  GetRelativePath(buffer, path, relativeTo, separator);
  return String(buffer);
}
inline String GetFolderString(const char* path) {
  char buffer[MAX_PATH_LENGTH];
  GetFolder(buffer, path);
  return String(buffer);
}
inline String GetFileNameString(const char* path) {
  char buffer[MAX_PATH_LENGTH];
  GetFileName(buffer, path);
  return String(buffer);
}
inline String GetFileBaseString(const char* path) {
  char buffer[MAX_PATH_LENGTH];
  GetFileBase(buffer, path);
  return String(buffer);
}
inline String GetFileExtensionString(const char* path) {
  char buffer[MAX_PATH_LENGTH];
  GetFileExtension(buffer, path);
  return String(buffer);
}
inline String SetFileExtensionString(const char* path, const char* extension) {
  char buffer[MAX_PATH_LENGTH];
  SetFileExtension(buffer, path, extension);
  return String(buffer);
}
...
inline String ToString(const _type_& value) {
  return String();
}

template <typename _type_>
inline String ToString(const _type_* valueArray, const int count) {
  String string = "{";
...
template <typename _type_>
inline String ToString(const Array<_type_>& valueArray) {
  String string = "{";
...
template <>
inline String ToString(const short& value) {
  return String(Va(" %hi", value));
}
template <>
inline String ToString(const unsigned short& value) {
  return String(Va(" %uhi", value));
}
template <>
inline String ToString(const int& value) {
  return String(Va(" %li", value));
}
template <>
inline String ToString(const unsigned int& value) {
  return String(Va(" %uli", value));
}
template <>
inline String ToString(const float& value) {
  return String(Va(" %f", value));
}
template <>
...
template <>
inline String ToString(const Vector2f& value) {
  return String(Va("{ %f %f }", value.x, value.y));
}
template <>
inline String ToString(const Vector2d& value) {
  return String(Va("{ %f %f }", value.x, value.y));
}
template <>
inline String ToString(const Vector2i& value) {
  return String(Va("{ %d %d }", value.x, value.y));
}

template <>
inline String ToString(const Vector3f& value) {
  return String(Va("{ %f %f %f }", value.x, value.y, value.z));
}
template <>
inline String ToString(const Vector3d& value) {
  return String(Va("{ %f %f %f }", value.x, value.y, value.z));
}
template <>
inline String ToString(const Vector3i& value) {
  return String(Va("{ %d %d %d }", value.x, value.y, value.z));
}

template <>
inline String ToString(const Vector4f& value) {
  return String(Va("{ %f %f %f %f }", value.x, value.y, value.z, value.w));
}
template <>
inline String ToString(const Vector4d& value) {
  return String(Va("{ %f %f %f %f }", value.x, value.y, value.z, value.w));
}
template <>
inline String ToString(const Vector4i& value) {
  return String(Va("{ %d %d %d %d }", value.x, value.y, value.z, value.w));
}

template <>
inline String ToString(const Matrix4f& value) {
  return String(
...
template <>
inline String ToString(const Matrix4d& value) {
  return String(
      Va("{ %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f }",
         value.M[0][0],
         value.M[0][1],
...
template <>
inline String ToString(const Quatf& value) {
  return String(Va("{ %f %f %f %f }", value.x, value.y, value.z, value.w));
}
template <>
inline String ToString(const Quatd& value) {
  return String(Va("{ %f %f %f %f }", value.x, value.y, value.z, value.w));
}

template <>
inline String ToString(const Planef& value) {
  return String(Va("{ %f %f %f %f }", value.N.x, value.N.y, value.N.z, value.D));
}
template <>
inline String ToString(const Planed& value) {
  return String(Va("{ %f %f %f %f }", value.N.x, value.N.y, value.N.z, value.D));
}

template <>
inline String ToString(const Bounds3f& value) {
  return String(
      Va("{{ %f %f %f }{ %f %f %f }}",
         value.b[0].x,
         value.b[0].y,
...
template <>
inline String ToString(const Bounds3d& value) {
  return String(
      Va("{{ %f %f %f }{ %f %f %f }}",
         value.b[0].x,
         value.b[0].y,
...
template <typename _type_>
inline size_t StringTo(Array<_type_>& valueArray, const char* string) {
  size_t length = 0;
  length += strspn(string + length, "{ \t\n\r");
  for (;;) {
    _type_ value;
