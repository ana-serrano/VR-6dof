...
class JSON : public RefCountBase<JSON>, public ListNode<JSON> {
 ...
 public:
  ...
  const JSON* GetFirstItem() const {
    return (!Children.IsEmpty()) ? Children.GetFirst() : 0;
  }
  JSON* GetLastItem() {
    return (!Children.IsEmpty()) ? Children.GetLast() : 0;
  }
  const JSON* GetLastItem() const {
    return (!Children.IsEmpty()) ? Children.GetLast() : 0;
  }
  ...
  JSON* GetNextItem(JSON* item) {
    return Children.IsNull(item->pNext) ? 0 : item->pNext;
  }
  const JSON* GetNextItem(const JSON* item) const {
    return Children.IsNull(item->pNext) ? 0 : item->pNext;
  }
  JSON* GetPrevItem(JSON* item) {
    return Children.IsNull(item->pPrev) ? 0 : item->pPrev;
  }
  const JSON* GetPrevItem(const JSON* item) const {
    return Children.IsNull(item->pPrev) ? 0 : item->pPrev;
  }
...
class JsonReader {
 public:
  JsonReader(const JSON* json)
      : Parent(json), Child(json != NULL ? json->Children.GetFirst() : NULL) {}
  ...
  bool IsObject() const {
    return Parent != NULL && Parent->Type == JSON_Object;
  }
  bool IsArray() const {
    return Parent != NULL && Parent->Type == JSON_Array;
  }
  bool IsEndOfArray() const {
    OVR_ASSERT(Parent != NULL);
    return Parent->Children.IsNull(Child);
  }

  JSON const* GetFirstChild() const {
    return Parent->Children.GetFirst();
  }
  JSON const* GetNextChild(JSON const* child) const {
    return child->pNext;
  }
  ...
  const String GetChildStringByName(const char* childName, const String& defaultValue = String(""))
      const;
