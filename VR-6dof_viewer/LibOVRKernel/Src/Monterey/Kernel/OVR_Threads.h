...
/*
    WaitCondition is a synchronization primitive that can be used to implement what is known as a
   monitor.
    Dependent threads wait on a wait condition by calling Wait(), and get woken up by other threads
   that
    call Notify() or NotifyAll().

    The unique feature of this class is that it provides an atomic way of first releasing a Mutex,
   and then
    starting a wait on a wait condition. If both the mutex and the wait condition are associated
   with the same
    resource, this ensures that any condition checked for while the mutex was locked does not change
   before
...
class Thread {
 public:
  ...
  // An option to start a thread is available, but it should not be used if classes are derived from
  // Thread.
  // "processor" parameter specifies which hardware processor this thread will be run on.
  // -1 means OS decides this. Implemented only on Win32
  explicit Thread(
  ...
  threadReturn_t GetExitCode() const {
    return ExitCode;
  }
...
class ThreadRefCounted : public Thread, public RefCountBase<ThreadRefCounted> {
 public:
  ThreadRefCounted(size_t stackSize = 128 * 1024, int processor = -1)
      : Thread(stackSize, processor) {}
  ThreadRefCounted(
      ThreadFn threadFunction,
      void* userHandle = 0,
      size_t stackSize = 128 * 1024,
      int processor = -1,
      ThreadState initialState = NotRunning)
      : Thread(threadFunction, userHandle, stackSize, processor, initialState) {}
  // Constructors that initialize the thread with a create parameters structure.
