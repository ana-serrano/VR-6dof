...
bool Intersect_RayBounds(
    ...
    float& t0,
    float& t1) {
  const float rcpDirX = (fabsf(rayDir.x) > Math<float>::SmallestNonDenormal)
      ? (1.0f / rayDir.x)
      : Math<float>::HugeNumber;
  const float rcpDirY = (fabsf(rayDir.y) > Math<float>::SmallestNonDenormal)
      ? (1.0f / rayDir.y)
      : Math<float>::HugeNumber;
  const float rcpDirZ = (fabsf(rayDir.z) > Math<float>::SmallestNonDenormal)
      ? (1.0f / rayDir.z)
      : Math<float>::HugeNumber;
