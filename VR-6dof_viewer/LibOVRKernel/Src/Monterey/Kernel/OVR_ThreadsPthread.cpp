...
Thread::Thread(
    Thread::ThreadFn threadFunction,
    void* userHandle,
    size_t stackSize,
    int processor,
    Thread::ThreadState initialState) {
...
void* Thread_PthreadStartFn(void* phandle) {
  ...
  pthread->Finish();
  // We always returns null rather than casting int->pointer to avoid potential issues when going to
  // 64-bit.
  // This sacrifices the error code but avoids issues in case someone actually tries to fetch the
  // value
...
int Thread::GetOSPriority(ThreadPriority p) {
#ifdef OVR_OS_PS3
  switch (p) {
    case Thread::CriticalPriority:
      return 0;
    case Thread::HighestPriority:
      return 300;
    case Thread::AboveNormalPriority:
      return 600;
    case Thread::NormalPriority:
      return 1000;
    case Thread::BelowNormalPriority:
      return 1500;
    case Thread::LowestPriority:
      return 2500;
    case Thread::IdlePriority:
      return 3071;
    default:
      return 1000;
  }
  ...
  switch (p) {
    case Thread::CriticalPriority:
      return minPriority + (maxPriority - minPriority) * 7 / 8;
    case Thread::HighestPriority:
      return minPriority + (maxPriority - minPriority) * 6 / 8;
    case Thread::AboveNormalPriority:
      return minPriority + (maxPriority - minPriority) * 5 / 8;
    case Thread::NormalPriority:
      return minPriority + (maxPriority - minPriority) * 4 / 8;
    case Thread::BelowNormalPriority:
      return minPriority + (maxPriority - minPriority) * 3 / 8;
    case Thread::LowestPriority:
      return minPriority + (maxPriority - minPriority) * 2 / 8;
    case Thread::IdlePriority:
      return minPriority + (maxPriority - minPriority) * 1 / 8;
    default:
      return minPriority + (maxPriority - minPriority) * 4 / 8;
  }
#else
...
void Thread::SetThreadName(const char* name) {
  ...
  if (result != 0) {
    __android_log_print(
        ANDROID_LOG_WARN, "OVR_Threads", "SetThreadName %s failed %s", name, strerror(result));
  }
#endif
}
