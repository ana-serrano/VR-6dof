...
OVR_vsprintf(char* dest, size_t destsize, const char* format, va_list argList) {
  ...
  // overflowed
  // Android / Linux: -1 is only returned if a format error occured, bytesWritten == destsize means
  // buffer overflow
...
// invalid.
// Note: If you are planning on printing a string then it's more efficient to just use OVR_vsnprintf
// and
