...
void String::AppendString(const wchar_t* pstr, intptr_t len) {
  ...
  DataDesc* pnewData = AllocDataCopy1(oldSize + (size_t)encodeSize, 0, pdata->Data, oldSize);
...
void String::AppendString(const char* putf8str, intptr_t utf8StrSz) {
  ...
  SetData(AllocDataCopy2(
      oldSize + (size_t)utf8StrSz, 0, pdata->Data, oldSize, putf8str, (size_t)utf8StrSz));
  pdata->Release();
}
...
String& String::Insert(const char* substr, size_t posAt, intptr_t strSize) {
  ...
  size_t byteIndex = (poldData->LengthIsSize()) ? posAt : (size_t)UTF8Util::GetByteIndex(
                                                              posAt, poldData->Data, oldSize);
