...
#if !defined(OVR_LOG_VAARG_ATTRIBUTE)
#ifdef __GNUC__
// LDC - TODO
...
///
///      OVRError OVR_MAKE_SYS_ERROR(ovrResult code, ovrSysErrorCode sysCode, const char*
///      pDescription);
...
class OVRError {
 ...
 protected:
  ovrResult Code = 0; /// The main ovrResult, which is a high level error id.
  ovrSysErrorCode SysCode =
      0; /// May be ovrSysErrorCodeSuccess to indicate there isn't a relevant system error code.
  std::string Description; /// Unlocalized error description string.
  std::string
      Context; /// Context string. For example, for a file open failure this is the file path.
  std::string SourceFilePath; /// The __FILE__ where the error was first encountered.
  int SourceFileLine = 0; /// The __LINE__ where the error was first encountered.
};
